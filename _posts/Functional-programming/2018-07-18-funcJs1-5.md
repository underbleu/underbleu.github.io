---
layout: post
title: 1-5. 함수형 자바스크립트의 값, 불변성/영속성, 컬렉션
...
for(const v of arrayLike) log(v) // TypeError: arrayLike is not iterable
...
최신 브라우저에서는 열거 가능한(enumerable) 속성이 있는 값들은 Iterable/Iterator 프로토콜을 잘 따르고 있음. 하지만, arguments와 nodeList는 well-formed 이터러블은 아니다 !
...
* `Object.values` - 객체 자신의 속성 중 열거 가능한(enumerable) 속성의 속성 값을 배열로 반환
...
# 1. 함수형 자바스크립트의 값
...
> 따라서 자바스크립트에서 순회할 때에는 `for...of`를 쓰도록 할 것
...
* `partyMember` 생성
  ...
  * 원본의 값을 참조한 것이다 -> `[ member[1], member[2] ]`
...
# 3. 컬렉션 중심 프로그래밍
최근 자바스크립트로 처리해야하는 event와 data의 스케일이 커졌다. 이러한 Remote data를 비동기처리하기 위한 여러 Functional 패러다임들이 있다.

### # 1. 컬렉션 중심 프로그래밍
"내가 다루고자 하는 모든 로직을, 코드라는 **값**으로 다루겠다"
